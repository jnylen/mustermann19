require 'mustermann19/ast/expander'
require 'mustermann19/caster'
...
module Mustermann19
  ...
  #
  # @example
  #   expander = Mustermann19::Expander.new(additional_values: :append)
  ...
  class Expander
    ...
    # @param [Array<#to_str, Mustermann19::Pattern>] patterns list of patterns to expand, see {#add}.
    # @param [Symbol] additional_values behavior when encountering additional values, see {#expand}.
    # @param [Hash] options used when creating/expanding patterns, see {Mustermann19.new}.
    ...
    #
    # @example
    #   expander = Mustermann19::Expander.new
    #   expander.add("/:a.jpg", "/:b.png")
    #   expander.expand(a: "pony") # => "/pony.jpg"
    #
    # @param [Array<#to_str, Mustermann19::Pattern>] patterns list of to add for expansion, Strings will be compiled to patterns.
    # @return [Mustermann19::Expander] the expander
    def add(*patterns)
      patterns.each do |pattern|
        pattern = Mustermann19.new(pattern.to_str, @options) if pattern.respond_to? :to_str
    ...
    #     expander = Mustermann19::Expander.new('/users/:user_id')
    ...
    #     expander = Mustermann19::Expander.new('/users/:user_id', '/pages/:page_id', '/:image_id.jpg')
    #     expand.cast(:user, :page, :image) { |key, value| { "#{key}_id".to_sym => value.id } }
    #
    #     expand.expand(user: User.current) # => "/users/42"
    #
    #   @example casting to multiple key/value pairs
    #     expander = Mustermann19::Expander.new('/users/:user_id/:image_id.:format')
    ...
    #   pattern = Mustermann19::Expander.new('/:name', '/:name.:ext')
    ...
    #   pattern.expand(:raise,  name: 'hello', ext: 'png', scale: '2x') # raises Mustermann19::ExpandError
    #
    # @example Setting additional values behavior for the expander object
    #   pattern = Mustermann19::Expander.new('/:name', '/:name.:ext', additional_values: :append)
