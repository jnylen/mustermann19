...
``` ruby
if '/foo/bar' =~ Mustermann19.new('/foo/*')
  puts 'it works!'
end

case 'something.png'
when Mustermann19.new('foo/*') then puts "prefixed with foo"
when Mustermann19.new('*.pdf') then puts "it's a PDF"
when Mustermann19.new('*.png') then puts "it's an image"
end

pattern = Mustermann19.new('/:prefix/*.*')
...
You can create a pattern of a certain type by passing `type` option to `Mustermann19.new`:

``` ruby
require 'mustermann19'
pattern = Mustermann19.new('/*/**', type: :shell)
```
...
first  = Mustermann19.new('/foo/:input')
second = Mustermann19.new('/:input/bar')
...
pattern = Mustermann19.new('/home', type: :identity)
pattern.match('/')     # => nil
...
pattern = Mustermann19.new('/<prefix>/<int:id>', type: :flask)
...
``` ruby
pattern = Mustermann19.new('/:file(.:ext)?')
...
expander = Mustermann::Expander.new
expander << Mustermann19.new("/users/{user_id}", type: :template)
expander << Mustermann19.new("/pages/:page_id",  type: :rails)
```
...
pattern = Mustermann19.new(":name@:domain.:tld")
...
router = Mustermann::Router::Rack.new do
  ...
  on '/:name' do |env|
    name = env['Mustermann19.params']['name']
...
class MyObject
  def to_pattern(**options)
    Mustermann19.new("/foo", **options)
  end
end

object = MyObject.new
Mustermann19.new(object, type: :rails) # => #<Mustermann::Rails:"/foo">
```

It might also be that you want to call `to_pattern` yourself instead of `Mustermann19.new`. You can load `mustermann/to_pattern` to implement this method for strings, regular expressions and pattern objects:
...
``` ruby
pattern = Mustermann19.new('/auth/*', except: '/auth/login')
...
``` ruby
pattern = Mustermann19.new('/*anything', type: :rails, except: '/*anything.png')
...
``` ruby
pattern = Mustermann19.new(':a.:b', greedy: true)
pattern.match('a.b.c.d') # => #<MatchData a:"a.b.c" b:"d">

pattern = Mustermann19.new(':a.:b', greedy: false)
...
``` ruby
Mustermann19.new('a b') === 'a b'   # => true
Mustermann19.new('a b') === 'a%20b' # => true
```

You can avoid this by setting `uri_decode` to `false`:

``` ruby
Mustermann19.new('a b', uri_decode: false) === 'a b'   # => true
Mustermann19.new('a b', uri_decode: false) === 'a%20b' # => false
```
